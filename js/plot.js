      ...
      type: 'line',
       data: {
          datasets: [{
              ...
              data: [],
              pointRadius: 0,
              borderColor: particlesProperties['Na'].color,
              backgroundColor: particlesProperties['Na'].color,
          },{
              label: 'Cl',
              fill: false,
              data: [],
              pointRadius: 0,
              borderColor: particlesProperties['Cl'].color,
              backgroundColor: particlesProperties['Cl'].color,
              hidden: true,
          },{
              label: 'K',
              fill: false,
              data: [],
              pointRadius: 0,
              borderColor: particlesProperties['K'].color,
              backgroundColor: particlesProperties['K'].color,
              hidden: true,
          },{
              label: 'Net',
              fill: false,
...
setInterval(
  function() {
    ...
    if(!togLoop) { //If the plot is not paused

        for (i=0;i<4;i++) {

        if (i<3) {
          var particleType = particleTypes[i];
          // if ( particlesProperties[particleType]["display"]==false ) return;
